...
A Cursor skill that compresses your project memory files (`CURSOR.md`, todos, preferences) into caveman format — so every session loads fewer tokens automatically.

Cursor read `CURSOR.md` on every session start. If file big, cost big. Caveman make file small. Cost go down forever.
...
This skill is part of the [caveman](https://github.com/JuliusBrussee/caveman) toolkit — making the agent use fewer tokens without losing accuracy.

- **caveman** — make Cursor *speak* like caveman (cuts response tokens ~65%)
- **caveman-compress** — make Cursor *read* less (cuts context tokens ~46%)
